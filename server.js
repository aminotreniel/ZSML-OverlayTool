...
const { initializeFirebase, savePreviousMatchDraft, getPreviousMatchDrafts, getDraftById, deleteDraft } = require('./firebaseConfig');
...
const defaultMatchData = {
    "teamdata": {
        "blueteam": { "teamname": "BLUE TEAM", "score": "0", "logo": "", "playerlist": [{},{},{},{},{}] },
        "redteam": { "teamname": "RED TEAM", "score": "0", "logo": "", "playerlist": [{},{},{},{},{}] }
    }
};
...
const defaultMapDrawData = {
    "drawdata": { "status": "idle", "mode": "random", "fixedChoice": null, "result": null, "timestamp": 0 }
};
if (!fsSync.existsSync(mapDrawPath)) fsSync.writeFileSync(mapDrawPath, JSON.stringify(defaultMapDrawData, null, 2));
...
if (!fsSync.existsSync(mvpDataPath)) fsSync.writeFileSync(mvpDataPath, JSON.stringify(defaultMvpData, null, 2));
...
if (!fsSync.existsSync(notifPath)) fsSync.writeFileSync(notifPath, JSON.stringify(defaultNotifData, null, 2));

// 8. SCHEDULE DATA
const schedulePath = path.join(dbDir, 'schedule.json');
const defaultScheduleData = {
    "1": { show: false, time: "", logo1: "", team1: "", score1: "", logo2: "", team2: "", score2: "" },
    "2": { show: false, time: "", logo1: "", team1: "", score1: "", logo2: "", team2: "", score2: "" },
    "3": { show: false, time: "", logo1: "", team1: "", score1: "", logo2: "", team2: "", score2: "" },
    "4": { show: false, time: "", logo1: "", team1: "", score1: "", logo2: "", team2: "", score2: "" }
};
if (!fsSync.existsSync(schedulePath)) fsSync.writeFileSync(schedulePath, JSON.stringify(defaultScheduleData, null, 2));
...
app.get('/api/matchdraft', async (req, res) => {
    try {
        ...
        res.json(JSON.parse(data));
    } catch (error) { res.status(500).json({ message: 'Error reading draft data' }); }
});
...
app.post('/api/matchdraft', async (req, res) => {
    ...
    } catch (error) { res.status(500).json({ message: 'Error saving draft data' }); }
});
...
app.get('/api/previousdraft', async (req, res) => {
    try {
        ...
        res.json(JSON.parse(data));
    } catch (error) { res.status(500).json({ message: 'Error reading previous draft data' }); }
});
...
app.post('/api/archive-draft', async (req, res) => {
    try {
        ...
        // 2. Tulis ke file local previousmatchdraft.json (backup lokal)
        await fs.writeFile(prevDraftPath, currentDraftRaw);
        
        // 3. Save to Firebase
        const firebaseResult = await savePreviousMatchDraft(currentDraft);
        
        // 4. Broadcast agar Analyzer refresh otomatis
        broadcast({ type: 'analyzer_update' });
        
        if (firebaseResult.success) {
            res.json({ 
                message: 'Draft archived successfully to both local and Firebase',
        ...
        console.error(error);
        res.status(500).json({ message: 'Error archiving draft', error: error.message });
    }
});

// [BARU] Get all previous drafts from Firebase
app.get('/api/previous-drafts', async (req, res) => {
    try {
        const limit = parseInt(req.query.limit) || 10;
        const drafts = await getPreviousMatchDrafts(limit);
        res.json({ success: true, drafts: drafts, count: drafts.length });
    } catch (error) {
        console.error(error);
        res.status(500).json({ success: false, message: 'Error fetching drafts from Firebase', error: error.message });
    }
});

// [BARU] Get specific draft by ID from Firebase
app.get('/api/previous-drafts/:draftId', async (req, res) => {
    try {
...
app.delete('/api/previous-drafts/:draftId', async (req, res) => {
    try {
        const { draftId } = req.params;
...
app.get('/api/mapdraw', async (req, res) => {
    try {
        ...
        res.json(JSON.parse(data));
    } catch (error) { res.status(500).json({ message: 'Error reading map draw data' }); }
});
...
app.post('/api/mapdraw', async (req, res) => {
    ...
    } catch (error) { res.status(500).json({ message: 'Error saving map draw' }); }
});
...
app.post('/api/postgame', async (req, res) => {
    ...
    } catch (error) { res.status(500).json({ message: 'Error saving postgame data' }); }
});
...
app.get('/api/mvp', async (req, res) => {
    try {
        ...
        res.json(JSON.parse(data));
    } catch (error) { res.status(500).json({ message: 'Error reading MVP data' }); }
});
